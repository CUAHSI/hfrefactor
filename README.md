...
- `reconciled_flowpaths`: per-flowline polygons matching the new
  network.
- `refactored_divides`: unioned divides aligned to the refactored
  flowpaths.
- `lookup_table`: mapping between reconciled IDs and member COMIDs.
- `outlets` and `pois`: optional tables when POI/event data are
  provided.
...
- Flowlines should follow the NHDPlus schema (or similar) so the package
  can normalize field names (e.g., `COMID`, `toCOMID`, `LevelPathI`,
  `TotDASqkm`).
- Divides require a `FEATUREID` column that matches the base COMIDs in
  the flowline network.
- Points-of-interest must carry identifiers that align with the original
  (pre-refactor) flowline IDs.

## Contributing

Issues and pull requests are welcome. Please open an issue to discuss
substantial changes so we can align on scope, data requirements, and
